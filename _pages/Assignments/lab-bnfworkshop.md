...
This **lab** is your first practice at writing grammars, done while the stakes are low.  You write two small EBNF grammars, classify five sample languages by Chomsky level, and make one design argument, on paper or in a Markdown file, with a partner.  Grammar writing is the skill the Parser assignment leans on hardest, and the Grammar and Derivations Workshop later asks you to write a grammar for the real class language; this lab is the warm-up on toy examples.  A grammar is a set of rules that says which strings belong to a language.  BNF (Backus-Naur Form) is a notation for writing those rules.  EBNF (Extended BNF) adds operators for repetition, optional parts, and grouping, so the same rules take fewer lines.

**Pair policy.**  You may do this lab in pairs.  One partner proposes a production (a single grammar rule), and the other tries to break it with a string the rule handles wrongly.  Submit one document between you, with each of you naming the other, and you both earn the same grade.  Working alone is fine too.  See the course schedule for the assigned and due dates.  Derivation trees and ambiguity get their own treatment later, in class and in the Grammar and Derivations Workshop; here the job is to write grammars that draw the right boundary between strings that belong to the language and strings that do not.
...
> **Time budget.**  About two hours of grammar work plus the write-up: fifteen minutes for Part 0, about an hour for Part 1 (the configuration language takes longer than the phone directory), and twenty to thirty minutes each for Parts 2 and 3.  The last reflection prompt asks how long it actually took.

### Your First 15 Minutes

1.  Read the EBNF reference and the worked grammar below.  Follow the leftmost derivation one line at a time until you can say which production each step used.
2.  Pick your Part 0 language (signed decimal numbers, or boolean expressions with `and`, `or`, and `not`) and write its first production on paper, starting from the whole thing: `<number> ::= ...` or `<expr> ::= ...`.
3.  Hand the rule to your partner (or play both roles).  Find one string the rule accepts that it should not, or rejects that it should not, and fix the rule.  That loop (write a rule, attack it with a string, fix the rule) is the whole workflow of this lab.
...
Two habits keep grammars honest.  Every nonterminal that appears on a right side must have its own production; an undefined nonterminal is the most common lost point in Part 1.  And "one or more" is `<x> { <x> }`, not `{ <x> }`, which also accepts nothing at all.

### A Worked Grammar to Pattern On

The language is a bracketed list of one or more unsigned integers, such as `[7]` or `[12,3]`.  Part 0 asks you to do exactly this sequence (BNF, then EBNF, then a derivation) for a language of your own.  The BNF version uses recursion for "one or more":
...
Verification by hand, in the form Part 1 asks for.  Accepts: `[7]` (one item, one digit), `[12,3]` (derived above), `[0,0,0]` (repetition runs twice).  Rejects: `[]` (the `<items>` rule requires at least one `<int>`), `[1,]` (after `,` the second choice for `<items>` demands another `<items>`, and no production yields the empty string).  Each rejection names the production that blocks the string; that is the argument Parts 0 and 1 want.  This grammar says nothing about spaces, so `[12, 3]` is not in its language.  Your Part 1 languages do contain spaces (`NAME: (610) 555-0123`), so decide how you treat them and say so in one line: either write them into the productions where they occur, or state that spaces between tokens are ignored.
...
Do this part first, before the rest of the lab, and alone if you like.  Writing even a tiny grammar forces the decisions the reading makes look obvious: what counts as a terminal, where the recursion goes, and what the notation is buying you.  Bring the grammar you draft to class with the rule you are least sure about marked; rough edges are expected, and that uncertain rule is usually the best discussion of the day.
...
> 5. Rewrite the grammar with `{ }` for repetition, `[ ]` for optional parts, and `( )` for grouping, wherever one of them replaces a recursive or duplicated rule.  Note in one sentence exactly what the EBNF made shorter: the rule that disappeared or the choice that collapsed.
> 6. Choose a grammar from the Syntax and BNF/EBNF or Grammars activity and say which one.  Choose a string it generates that is three to six symbols long, and write its leftmost derivation one step per line, always expanding the leftmost nonterminal, with each line labeled by the production you used, as in the worked example.
> 7. Write a string that looks close to that language but is not in it (a near miss is more convincing than random characters), and explain how you know: name the production that would have to produce the offending symbol, and say why no production can.
> 8. Name the rule you are least sure about.
...
Lines of the form `NAME: (610) 555-0123` or `NAME: 555-0123`, where a name is one or more capitalized words.  The area code is optional; the punctuation is not.  Both of these lines belong to the language:
...
> 2. Define `<name>` as one or more capitalized words, and make the area code optional with `[ ]` while keeping its parentheses and the hyphen required.
> 3. Define every nonterminal you used, down to terminals: `<word>`, `<capital>`, `<letter>`, `<digit>`.
> 4. Decide how spaces are handled (in the productions, or ignored between tokens) and write that down in one line.
...
> **Do this.**
> 1. Start at the top with `<config> ::= { <line> }`, since zero lines is a legal configuration, and define `<line>` as a key, `=`, a value, and the required `;`.
> 2. Define `<key>` as an identifier: a letter or underscore, then any mix of letters, digits, and underscores.
> 3. Define `<value>` with three alternatives: integer, quoted string, and list.  Define the list in terms of `<value>` again; that recursion is what makes `["dark", ["contrast", "high"]]` legal.
> 4. Decide whether an empty list `[]` is legal and make the grammar say so with `[ ]` around the item sequence, or leave it out on purpose.
> 5. Define every remaining nonterminal down to terminals, and note your spacing decision as in Step 1.1.
...
> 3. For each accepted string, note the path through the productions in a few words.  For each rejected string, name the production that blocks it, as in the worked example.

---

## Part 2: Chomsky Classification (27%)

The Chomsky hierarchy ranks languages by how much memory a machine needs to recognize them.  Regular languages need only finite memory (a parity bit, "have I seen the first character yet").  Context-free languages need a stack, because they match or nest symbols that must be unwound in reverse order.  Languages with cross-serial constraints, such as equal counts in three separate places, need more than a stack.  For each language below, name the lowest Chomsky level that can describe it, with a one-sentence reason that names the structural property forcing that level.
...
> 2. For each language, ask what a recognizer must remember while reading the string, and write that structural property as the reason, in one sentence.  "It is context-free because it is a context-free language" restates the level name; "balanced parentheses need a stack to match each `)` to its `(`" names the property, and the rubric rewards the second form.
> 3. Close with one sentence for each of these three questions.  Which level do the class language's tokens (an identifier, a number, a keyword) need?  Which level does its full syntax (nested expressions, blocks inside blocks) need?  What does that split tell you about why compilers have both a lexer and a parser?  The third sentence should connect the first two.

---

## Part 3: Design-Criteria Argument (18%)

The configuration language's designer proposes making the trailing `;` optional.  Write one paragraph that evaluates the proposal against at least two of the readability, writability, and reliability criteria from the Evaluating Languages session.
...
|------------------|---------------|------------|
| `## Part 0` section | Your BNF and EBNF grammar, the note on what got shorter, a leftmost derivation, a non-generated string with an argument, and the rule you are least sure about | Part 0 (10%) |
| `## Part 1` section | Two complete EBNF grammars, each with three accepted and two rejected strings verified by hand and a line on spaces | EBNF Grammars (45%) |
...
- [ ] In every grammar, every nonterminal on a right-hand side has its own production, and repetition and optionality use `{ }` and `[ ]`, not words like "one or more".
- [ ] Each Part 1 grammar has three accepted and two rejected strings checked against the productions by hand, and the configuration grammar accepts the nested sample line.
